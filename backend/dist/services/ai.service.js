...
class AIService {
    ...
    constructor() {
        this.openai = new openai_1.default({
            apiKey: env_1.env.OPENAI_API_KEY || 'missing-api-key',
        });
        this.model = env_1.env.OPENAI_MODEL;
    }
    hasUsableApiKey() {
...
Analyze the code and provide detailed feedback.`;
            ...
            logger_1.logger.error('AI evaluation failed; using fallback feedback', {
                error,
     ...
     */
    async generateInterviewQuestion(input) {
        if (!this.hasUsableApiKey()) {
...
Generate a unique, engaging question.`;
            ...
            logger_1.logger.error('Interview question generation failed; using fallback question', { error, input });
            return this.fallbackInterviewQuestion(input);
        }
    }
    /**
     ...
     */
    async evaluateInterviewAnswer(question, answer, expectedTopics) {
        if (!this.hasUsableApiKey()) {
...
Evaluate this response.`;
            ...
            logger_1.logger.error('Interview answer evaluation failed; using fallback evaluation', {
                error,
                question,
            });
            return this.fallbackInterviewEvaluation(answer, expectedTopics);
...
Extract all relevant information and skills.`;
            ...
            logger_1.logger.error('Resume analysis failed; using fallback parser', { error });
            return this.fallbackResumeAnalysis(input.resumeText);
        }
    }
    /**
     ...
     */
    async generateLearningRecommendations(weakSkills, strongSkills, recentAttempts) {
        if (!this.hasUsableApiKey()) {
...
Generate personalized learning recommendations.`;
            ...
            logger_1.logger.error('Learning recommendations generation failed; using fallback recommendations', {
                error,
                recentAttempts,
            });
            return this.fallbackLearningRecommendations(weakSkills, strongSkills);
     ...
     */
    async generateInterviewSummary(transcript, questions) {
        if (!this.hasUsableApiKey()) {
...
Generate a comprehensive interview summary.`;
            ...
            logger_1.logger.error('Interview summary generation failed; using fallback summary', { error, transcript });
            return this.fallbackInterviewSummary(questions);
        }
    }
    /**
     ...
     */
    validateFeedbackOutput(feedback) {
        return {
    ...
    fallbackInterviewQuestion(input) {
    ...
    fallbackInterviewEvaluation(answer, expectedTopics) {
        const normalized = answer.toLowerCase();
        const coveredTopics = expectedTopics.filter((topic) => normalized.includes(topic.toLowerCase().split(' ')[0]));
    ...
    fallbackLearningRecommendations(weakSkills, strongSkills) {
    ...
    fallbackInterviewSummary(questions) {
